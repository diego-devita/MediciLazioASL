...
const log = createLogger('medici:client');
...
export class MediciSearchClient {
  ...
  async getConfig(forceRefresh = false) {
    ...
    if (this.useStaticConfig) {
      ...
      log.debug('Uso configurazione statica', this.config);
      return this.config;
    }
    ...
    if (forceRefresh || !this.config || isExpired) {
      log.debug('Recupero nuova configurazione...');
      this.config = await this._fetchConfig();
      this.configTimestamp = now;
    } else {
      log.debug('Uso configurazione cachata');
    }
  ...
  async _fetchConfig() {
    try {
      ...
      if (!layoutId) {
        log.warn('Layout ID non trovato dinamicamente, uso default 16262');
        layoutId = '16262';
      }
      ...
      log.debug('Configurazione trovata', config);
      return config;

    } catch (error) {
      log.error('Errore nel recupero configurazione', error);
      return {
        layoutId: '16262',
        instanceId: 'gIo787T487Lc',
        fullInstanceId: 'it_smc_laziocrea_ums_web_internal_portlet_GenericListPortlet_INSTANCE_gIo787T487Lc'
  ...
  buildParams(searchParams = {}) {
    ...
    log.trace('buildParams chiamato con', searchParams);
    ...
    const systemParams = {
      ...
      'p_p_col_count': '2',
      'p_p_isolated': '1',
      'currentURL': '%2Fricerca-medici',  // URL encoded come fa il portale originale
      'portletAjaxable': '1'
    };
    ...
    // IMPORTANTE: Il portale Lazio RICHIEDE che tutti i parametri siano presenti,
    // anche se vuoti (es: surname=&name=&zip=&)
    // NON rimuovere i parametri vuoti o la ricerca fallirà!
    ...
    log.trace('Query string generata:', queryString.substring(0, 300) + '...');
  ...
  async searchMedici(searchParams = {}, options = {}) {
    ...
    log.debug('Parametri ricerca', { searchParams, queryString: params });
    ...
    try {
      ...
      log.error('Errore nella ricerca', error);
      throw error;
    }
  }
  ...
  async searchMediciAllPages(searchParams = {}, options = {}) {
    ...
    log.debug(`Trovate ${totalPages} pagine totali`);
    ...
    if (totalPages > maxPages) {
      log.debug(`⚠️ Limitando scan a ${maxPages} pagine su ${totalPages} totali per evitare timeout`);
    }
    ...
    for (let page = 2; page <= pagesToFetch; page++) {
      log.debug(`Fetching pagina ${page}/${pagesToFetch}`);
    ...
    log.debug(`Totale risultati aggregati: ${allResults.length}`);
    return allResults;
  }
  ...
  async _fetchPage(searchParams, options = {}) {
    ...
    // Logging dettagliato della richiesta al portale fonte
    const requestDetails = {
      url: this.urls.portlet,
      method: 'POST',
      headers: {
        ...headers,
        // Nascondi eventuali cookie per privacy
        cookie: headers.cookie ? '[REDACTED]' : undefined
      },
      referrer: this.urls.search,
      searchParams: searchParams,
      bodyFull: params, // Body completo per debug
      bodyLength: params.length,
      timestamp: new Date().toISOString()
    };

    log.trace('🌐 Richiesta API Portale Lazio:', requestDetails);
    ...
    const responseDetails = {
      ...
      bodyLength: htmlText.length,
      bodyFull: htmlText, // HTML completo per debug
      timestamp: new Date().toISOString()
    };

    log.trace('📥 Risposta API Portale Lazio:', responseDetails);

    if (!response.ok) {
      log.error('❌ Errore HTTP dal Portale Lazio:', {
        status: response.status,
        statusText: response.statusText,
  ...
  _extractTotalPages(html) {
    try {
      ...
      log.error('Errore nell\'estrazione del numero di pagine:', e);
      return 1;
    }
  }
  ...
  async searchMedici(surnames = [], commonParams = {}, options = {}) {
    ...
    log.debug(`Generazione di ${combinations.length} combinazioni da ricercare`);
    log.debug(`Cognomi: ${surnamesList.length}, Nomi: ${namesList.length}, CAP: ${zipsList.length}`);
    ...
    if (options.page) {
      ...
      log.debug(`Ricerca pagina ${options.page} per: cognome=${surname}, nome=${name || '(vuoto)'}, cap=${zip || '(vuoto)'}`);
      ...
      log.debug(`Trovati ${medici.length} risultati nella pagina ${options.page} di ${totalPages}`);
    } else {
      ...
      for (const [surname, name, zip] of combinations) {
        try {
          ...
          log.debug(`Ricerca combinazione: cognome=${surname}, nome=${name || '(vuoto)'}, cap=${zip || '(vuoto)'}`);

          // Usa searchMediciAllPages per aggregare tutte le pagine
          const medici = await this.searchMediciAllPages(searchParams, options);
          allMedici.push(...medici);

          log.debug(`Trovati ${medici.length} risultati per questa combinazione`);
        } catch (error) {
          log.error(`Errore ricerca combinazione cognome=${surname}, nome=${name}, cap=${zip}:`, error);
    ...
    log.debug(`Totale risultati da tutte le combinazioni: ${allMedici.length}`);
    log.debug(`Totale risultati unici (deduplicati): ${uniqueMedici.length}`);
  ...
  // ===== PARSING RISULTATI =====
  parseMedici(html) {
    log.trace('🔍 Inizio parsing HTML risposta...');
    ...
    log.trace(`📊 Trovate ${rows.length} righe nella tabella risultati`);

    rows.forEach((row, index) => {
      const cells = row.querySelectorAll('td');

      // Trace: log dettagliato delle prime 3 righe
      if (index < 3) {
        log.trace(`Riga ${index + 1} - ${cells.length} celle:`,
          Array.from(cells).map((cell, i) => `[${i}]="${cell.textContent.trim().substring(0, 50)}"`).join(', ')
        );
      }

      // Debug: log del numero di celle per la prima riga (backward compat)
      if (index === 0 && this.debug) {
        log.debug(`Prima riga ha ${cells.length} celle:`);
        cells.forEach((cell, i) => {
          log.debug(`  Cella ${i}: "${cell.textContent.trim()}"`);
        });
      }
      ...
      if (cells.length >= 5) {
        ...
        // Trace: log delle prime 3 righe parsate
        if (index < 3) {
          log.trace(`✅ Medico ${index + 1} parsato:`, {
            nome: medico.nome,
            cognome: medico.cognome,
            asl: medico.asl,
            assegnabilita: medico.assegnabilita,
            tipo: medico.tipo,
            codiceFiscale: medico.codiceFiscale,
            identificativo: medico.identificativo
          });
        }
    ...
    log.trace(`✅ Parsing completato: ${medici.length} medici estratti dall'HTML`);

    // Prova ad aggiungere dati extra dal JavaScript
    this._enhanceMediciData(html, medici);

    log.trace(`✅ Enhancement completato: ${medici.length} medici totali`);
  ...
  _enhanceMediciData(html, medici) {
    try {
      ...
      if (!scriptMatch || !scriptMatch[1]) {
        log.debug('DataStore non trovato nel JavaScript');
        return;
      }
      ...
      if (this.debug && dataString.length > 0) {
        log.debug('DataStore trovato, primi 500 caratteri:', dataString.substring(0, 500));
      }
      ...
      for (const match of objectMatches) {
        ...
        if (index === 0 && this.debug) {
          log.debug('Primo oggetto nel dataStore:', objString);
        }
      ...
      log.error('Error enhancing medici data:', e);
    }
  }

  // ===== UTILITY =====
  // Removed: now using centralized logger
