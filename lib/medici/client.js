...
export class MediciSearchClient {
    ...
    rows.forEach(row => {
      ...
      if (cells.length >= 6) {
        ...
        if (detailLink) {
          ...
          const match = onclick.match(/'([A-Z0-9]{16})'/);
          if (match) codiceFiscale = match[1];
        }
  ...
  _enhanceMediciData(html, medici) {
    try {
      const scriptMatch = html.match(/searchContainer\.updateDataStore\(\["(.*?)"\]\);/);
      if (scriptMatch && scriptMatch[1] && medici[0]) {
        const dataString = scriptMatch[1];

        const extraFields = {
          email: dataString.match(/email=([^,}]+)/)?.[1] || null,
          indirizzo: dataString.match(/address=([^,}]+)/)?.[1] || null,
          luogo: dataString.match(/place=([^,}]+)/)?.[1] || null,
          identificativo: dataString.match(/identifier=([^,}]+)/)?.[1] || null,
          luogoNascita: dataString.match(/birthPlace=([^,}]+)/)?.[1] || null,
          codiceDistretto: dataString.match(/districtCode=([^,}]+)/)?.[1] || null,
          descrizioneDistretto: dataString.match(/districtDescription=([^,}]+)/)?.[1] || null
        };

        Object.assign(medici[0], extraFields);
      }
    } catch (e) {
