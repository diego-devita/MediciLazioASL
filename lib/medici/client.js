...
export class MediciSearchClient {
  ...
  async searchMediciAllPages(searchParams = {}, options = {}) {
    ...
    // Fetch tutte le altre pagine
    const allResults = [...firstPageResults];

    for (let page = 2; page <= totalPages; page++) {
      this.log(`Fetching pagina ${page}/${totalPages}`);
  ...
  async searchBySurnames(surnames = [], commonParams = {}) {
    ...
    for (const surname of surnames) {
      try {
        this.log(`Ricerca cognome: ${surname}`);
        const searchParams = { ...commonParams, surname };
        // Usa searchMediciAllPages per aggregare tutte le pagine
        const medici = await this.searchMediciAllPages(searchParams);
        allMedici.push(...medici);
      } catch (error) {
        console.error(`Errore ricerca cognome ${surname}:`, error);
        // Continua con gli altri cognomi anche se uno fallisce
      }
    }
    ...
    this.log(`Totale risultati unici: ${uniqueMedici.length}`);
    return uniqueMedici;
  }
