...
export async function logLoginAttempt(attemptData) {
  try {
    ...
    console.log(`Login attempt logged: ${attempt.ip} - ${attempt.success ? 'success' : 'failed'}`);
  } catch (error) {
...
export async function saveVariationHistory(chatId, variationData) {
  try {
    ...
    console.log(`Variation history saved for user ${chatId}`);
  } catch (error) {
 ...
 */
export async function getVariationHistory(chatId, limit = 50) {
  try {
    const { db } = await connectToDatabase();
    const variations = db.collection(DATABASE.COLLECTIONS.VARIATIONS_HISTORY);
