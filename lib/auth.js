...
export async function createJWT(payload = {}, expiryDays = null) {
  const secret = getJWTSecret();

  const jwt = new SignJWT({ ...payload, authenticated: true })
    .setProtectedHeader({ alg: 'HS256' })
    .setIssuedAt();

  // Aggiungi scadenza se specificata
  if (expiryDays) {
    jwt.setExpirationTime(`${expiryDays}d`);
  }
...
export async function validateJWT(req) {
  try {
    // Estrai token da cookie O da Authorization header
    let token = null;

    // Prova prima con Authorization header
...
export function requireAuth(handler) {
  return async (req, res) => {
    ...
    // Passa l'utente completo al handler (con tutti i campi dal DB)
    // Include anche i campi dal JWT payload (impersonatedBy, impersonatorRole)
    req.user = {
      ...authResult.user,
...
export function requireAdmin(handler) {
  return async (req, res) => {
    console.log('🔸 requireAdmin: method=', req.method, 'url=', req.url);

    // Skip auth for OPTIONS (CORS preflight)
    if (req.method === 'OPTIONS') {
      console.log('🔸 requireAdmin: OPTIONS, skipping auth');
      return handler(req, res);
    }

    const authResult = await validateJWT(req);
    console.log('🔸 requireAdmin: authResult.valid=', authResult.valid);

    if (!authResult.valid) {
      console.log('❌ requireAdmin: auth failed');
      return res.status(401).json({
        success: false,
    ...
    // Se sta impersonando, controlla l'impersonatorRole invece del role
    const effectiveRole = authResult.payload.impersonatorRole || authResult.payload.role;
    console.log('🔸 requireAdmin: role=', authResult.payload.role, 'effectiveRole=', effectiveRole);
    ...
    // Include anche i campi dal DB
    req.user = {
      ...authResult.user,
      ...authResult.payload
    };

    console.log('✅ requireAdmin: calling handler');
    return handler(req, res);
  };
}
